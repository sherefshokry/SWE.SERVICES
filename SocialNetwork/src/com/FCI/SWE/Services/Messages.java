...
public class Messages {
	 ...
	 if(msg.equals(""))
   {
 	...
 	public String AddMemberInChatGroup(
       ...
       FriendsServices Friendobj = new FriendsServices();
       String uName = Friendobj.searchUser(memberToAdd);
       if(uName == null)
       {
                 obj.put("status" ,"falied");
                 return obj.toString();
       }
 		...
 		for (Entity user : pq.asIterable()) {
 			...
 			if (user.getProperty("ID").toString().equals(ID)) {
               ...
               if(checkEmails(emails,memberToAdd))
               {
                 obj.put("status" ,"falied");
                 return obj.toString();
               }
   ...
   public boolean checkEmails(ArrayList<String> emails , String member)
   {
    
     for(int i=0;i<emails.size();i++)
       if(emails.get(i).equals(member))
            return true;
